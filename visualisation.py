...
def reconstruct_hsi_from_embeddings(model, original_tokens, predicted_tokens, mask):
    """
    Reconstruct HSI directly from embeddings using the model's weights.

    Args:
        model: The MultiModalSpectralGPT model
        original_tokens: Original token embeddings [B, L, D]
        predicted_tokens: Predicted token embeddings [B, L, D]
        mask: Binary mask indicating which tokens were masked [B, L]

    Returns:
        tuple: (reconstructed_hsi, original_hsi_with_mask, pixel_mask) as torch tensors
    """
    import torch

    # Get model parameters
    B, L, D = original_tokens.shape
    ...
    h_patches = H // patch_h
    w_patches = W // patch_w
    t_patches = T // t_patch_size

    # Get the projection weights from the patch embedding layer
    proj_weight = model.patch_embed.proj.weight  # [D, C, Tp, Ph, Pw]

    # Create a pseudo-inverse to reverse the embedding
    ...
    reconstructed_hsi = torch.zeros((B, 1, T, H, W), device=device)
    original_hsi_with_mask = torch.zeros((B, 1, T, H, W), device=device)
    pixel_mask = torch.zeros((B, 1, T, H, W), device=device)

    # Combine original and predicted tokens
    combined_tokens = torch.clone(original_tokens)
    for b in range(B):
        masked_indices = torch.where(mask[b] > 0.5)[0]
        combined_tokens[b, masked_indices] = predicted_tokens[b, masked_indices]

    # Also create version with only original tokens (for the masked visualization)
    original_only_tokens = torch.clone(original_tokens)

    # Loop through each batch and reconstruct the HSI volume
    for b in range(B):
        # Reshape tokens to 3D patch organization
        tokens_3d = combined_tokens[b].reshape(t_patches, h_patches, w_patches, D)
        original_tokens_3d = original_only_tokens[b].reshape(t_patches, h_patches, w_patches, D)

        # Process each token location
        for t_idx in range(t_patches):
            for h_idx in range(h_patches):
                for w_idx in range(w_patches):
                    # Get patch location indices
                    t_start = t_idx * t_patch_size
                    h_start = h_idx * patch_h
                    w_start = w_idx * patch_w

                    # Get token for this patch location
                    token = tokens_3d[t_idx, h_idx, w_idx]
                    orig_token = original_tokens_3d[t_idx, h_idx, w_idx]

                    # Apply pseudo-inverse to get patch pixels
                    patch_pixels = torch.matmul(pseudo_inv, token)
                    orig_patch_pixels = torch.matmul(pseudo_inv, orig_token)

                    # Reshape to proper patch dimensions
                    patch_pixels = patch_pixels.reshape(1, t_patch_size, patch_h, patch_w)
                    orig_patch_pixels = orig_patch_pixels.reshape(1, t_patch_size, patch_h, patch_w)

                    # Place in the reconstructed HSI volume
                    reconstructed_hsi[b, :,
                    t_start:t_start + t_patch_size,
                    h_start:h_start + patch_h,
                    w_start:w_start + patch_w] = patch_pixels

                    original_hsi_with_mask[b, :,
                    t_start:t_start + t_patch_size,
                    h_start:h_start + patch_h,
                    w_start:w_start + patch_w] = orig_patch_pixels

                    # Check if this patch was masked and update pixel mask
                    token_idx = t_idx * h_patches * w_patches + h_idx * w_patches + w_idx
                    if mask[b, token_idx] > 0.5:
                        pixel_mask[b, :,
                        t_start:t_start + t_patch_size,
                        h_start:h_start + patch_h,
                        w_start:w_start + patch_w] = 1.0

    # Apply a visual indicator to masked regions in the original visualization
    # (gray out masked regions)
    for b in range(B):
        for t in range(T):
            mask_slice = pixel_mask[b, 0, t]
            original_hsi_with_mask[b, 0, t] = (
                    original_hsi_with_mask[b, 0, t] * (1 - mask_slice) + 0.5 * mask_slice
            )

    return reconstructed_hsi, original_hsi_with_mask, pixel_mask


def visualize_reconstructed_hsi(model, val_batch, device, save_path=None):
    """
    Visualize the reconstructed HSI data from model embeddings as RGB images.

    Args:
        model: The MultiModalSpectralGPT model
        val_batch: A validation batch with HSI data
        device: The device to run the model on
        save_path: Path to save the visualization

    Returns:
        tuple: (figure, (reconstructed_hsi, original_hsi_with_mask, pixel_mask))
    """
    import torch
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Move data to device
    hsi = val_batch['hsi'].to(device)
    aux_data = {k: v.to(device) if isinstance(v, torch.Tensor) else v
                for k, v in val_batch['aux_data'].items()}
    batch_idx = val_batch['batch_idx'].to(device)
    ...
    predicted_tokens = output['pred']
    mask = output['mask']

    # Reconstruct HSI from embeddings
    reconstructed_hsi, original_hsi_with_mask, pixel_mask = reconstruct_hsi_from_embeddings(
        model, original_tokens, predicted_tokens, mask
    )

    # Normalize the reconstructed HSI values to a reasonable range
    # This is important because the pseudo-inverse reconstruction might produce values
    # outside the expected range

    # Find min/max values for each HSI cube
    for b in range(reconstructed_hsi.shape[0]):
        ...
        # Normalize original with mask similarly
        orig_min = original_hsi_with_mask[b].min()
        orig_max = original_hsi_with_mask[b].max()
        original_hsi_with_mask[b] = (original_hsi_with_mask[b] - orig_min) / (orig_max - orig_min + 1e-8)

    # Create visualization
    fig, axes = plt.subplots(1, 3, figsize=(20, 8))

    # Convert HSI to RGB for visualization
    orig_rgb = simple_hsi_to_rgb(hsi)
    recon_rgb = simple_hsi_to_rgb(reconstructed_hsi)
    orig_masked_rgb = simple_hsi_to_rgb(original_hsi_with_mask)

    # Convert tensors to numpy for plotting
    if len(orig_rgb.shape) == 5:  # [B, C, 3, H, W]
        orig_rgb_np = orig_rgb[0, 0].permute(1, 2, 0).cpu().numpy()
        recon_rgb_np = recon_rgb[0, 0].permute(1, 2, 0).cpu().numpy()
        orig_masked_rgb_np = orig_masked_rgb[0, 0].permute(1, 2, 0).cpu().numpy()
    elif len(orig_rgb.shape) == 4:  # [B, 3, H, W]
        orig_rgb_np = orig_rgb[0].permute(1, 2, 0).cpu().numpy()
        recon_rgb_np = recon_rgb[0].permute(1, 2, 0).cpu().numpy()
        orig_masked_rgb_np = orig_masked_rgb[0].permute(1, 2, 0).cpu().numpy()
    else:
        orig_rgb_np = orig_rgb.permute(1, 2, 0).cpu().numpy()
        recon_rgb_np = recon_rgb.permute(1, 2, 0).cpu().numpy()
        orig_masked_rgb_np = orig_masked_rgb.permute(1, 2, 0).cpu().numpy()

    # Clip values to [0, 1]
    orig_rgb_np = np.clip(orig_rgb_np, 0, 1)
    recon_rgb_np = np.clip(recon_rgb_np, 0, 1)
    orig_masked_rgb_np = np.clip(orig_masked_rgb_np, 0, 1)
    ...
    # Plot original with masked regions
    axes[1].imshow(orig_masked_rgb_np)
    axes[1].set_title("Original HSI with Masked Regions")
    axes[1].axis('off')

    # Plot reconstructed
    axes[2].imshow(recon_rgb_np)
    axes[2].set_title("Reconstructed HSI")
    axes[2].axis('off')
    ...
    return fig, (reconstructed_hsi, original_hsi_with_mask, pixel_mask)


def visualize_spectral_slice(original_hsi, reconstructed_hsi, pixel_mask, slice_idx=None, save_path=None):
    """
    Visualize a specific spectral slice to compare original vs. reconstructed HSI.

    Args:
        original_hsi: Original HSI data [B, C, T, H, W]
        reconstructed_hsi: Reconstructed HSI data [B, C, T, H, W]
        pixel_mask: Mask showing which areas were masked [B, C, T, H, W]
        slice_idx: Index of spectral slice to visualize (default: middle slice)
        save_path: Path to save the visualization
    ...
    # Select a middle spectral slice if not specified
    if slice_idx is None:
        slice_idx = original_hsi.shape[2] // 2

    # Create figure
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))

    # Get the slices
    orig_slice = original_hsi[0, 0, slice_idx].cpu().numpy()
    recon_slice = reconstructed_hsi[0, 0, slice_idx].cpu().numpy()
    mask_slice = pixel_mask[0, 0, slice_idx].cpu().numpy()

    # Calculate error between original and reconstructed
    # Only for masked regions
    error_slice = np.abs(orig_slice - recon_slice) * mask_slice

    # Display original
    axes[0].imshow(orig_slice, cmap='viridis')
    axes[0].set_title(f"Original (Spectral Band {slice_idx})")
    axes[0].axis('off')

    # Display reconstructed
    axes[1].imshow(recon_slice, cmap='viridis')
    axes[1].set_title(f"Reconstructed (Spectral Band {slice_idx})")
    axes[1].axis('off')

    # Display error in masked regions
    error_img = axes[2].imshow(error_slice, cmap='hot')
    axes[2].set_title("Reconstruction Error (Masked Regions)")
    axes[2].axis('off')
